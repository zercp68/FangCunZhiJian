...
public class WeaponSystem : Singleton<WeaponSystem>
{
    ...
    #region 计算系统的定义
    CombatSystem leftCombatSystem=new CombatSystem();
    CombatSystem rightCombatSystem=new CombatSystem();
    #endregion
    ...
    public WeaponCard Setup(WeaponCardData weaponCardData)
    {
        WeaponCard weaponCard=new WeaponCard(weaponCardData);
        return weaponCard;
    }
    ...
    public IEnumerator WeaponActionPerformer(WeaponActionGA weaponActionGA)
    {
        ...
        if (actionCard.isDefenseAction)
        {
            ...
            HeroSystem.Instance.UpdateShield(defenseValue);
            yield return null;
        }
        else
        {
